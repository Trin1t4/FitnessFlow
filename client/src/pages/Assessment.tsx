...
export default function Assessment() {
  ...
  // ✅ CALCOLO LIVELLO CORRETTO: 70% PRACTICAL + 30% PHYSICAL
  const calculateFinalLevel = (practicalExercises: AssessmentExercise[]): LevelResult => {
    ...
    if (hasBodyScan) {
      ...
      bmiScore = bmiValue >= 18.5 && bmiValue <= 25 ? 100 : 
                 bmiValue < 18.5 ? Math.max(0, 50 + (bmiValue - 18.5) * 10) :
                 Math.max(0, 100 - (bmiValue - 25) * 5);
      ...
      console.log(`[ASSESSMENT] 📊 BMI fallback: ${bmiValue.toFixed(1)} (${bmiScore.toFixed(0)}/100), Age ${age} (${ageScore}/100)`);
    }
    ...
    return {
      ...
      breakdown: {
        ...
        age,
        ageScore: age <= 30 ? 100 : age <= 40 ? 90 : age <= 50 ? 80 : age <= 60 ? 70 : 60
      }
    };
  };
          ...
          <div className="bg-slate-800/50 backdrop-blur-lg rounded-2xl p-8 border border-slate-700">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
              ...
              <div className="bg-slate-700/50 rounded-lg p-4 border border-slate-600">
                ...
                <p className="text-xs text-amber-400 mt-2">ℹ️ Solo informativo</p>
              </div>
              
              {/* PRACTICAL - 70% */}
              <div className="bg-emerald-500/20 rounded-lg p-4 border border-emerald-500/50">
                <p className="text-xs text-emerald-300 mb-1">💪 Test Pratici (70%)</p>
              ...
              <div className="bg-blue-500/20 rounded-lg p-4 border border-blue-500/50 md:col-span-2">
                <p className="text-xs text-blue-300 mb-1">
                  📊 Parametri Fisici (30%)
                  {levelResult.breakdown.bodyFat !== 'N/A' && ' - 📸 Da Body Scan'}
                </p>
            ...
            <div className="bg-slate-700/30 rounded-lg p-5 mb-6">
              <p className="text-sm text-slate-300 leading-relaxed">
                Il tuo livello <strong className="text-white">{levelText}</strong> è stato calcolato combinando la tua performance nei test pratici (70%) con i parametri fisici età e {levelResult.breakdown.bodyFat !== 'N/A' ? 'body fat da foto' : 'BMI'} (30%). 
